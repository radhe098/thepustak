...
const Categoriesdemo = () => {
  ...
  }, []); // Removed 'books' from dependency array to avoid infinite loop

  const handleBookClick = (file) => {
            ...
            {books.map((book) => {
              return (
                <div key={book.id} className="text-lg flex-wrap flex justify-between">
                  <div className="flex flex-col justify-between items-center m-4 w-40 rounded-lg hover:scale-105 transform transition-transform duration-300 bg-teal-200 h-[14rem] border-black">
                    <div className="h-48 ml-3 cursor-pointer" onClick={() => handleBookClick(book)}>
